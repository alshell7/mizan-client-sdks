...
The distribution name is `mizan-billing`, version `1.2.0`; the import is `mizan`.
...
5. Push tag `mizan-python-v1.2.0`. GitHub Actions builds once and publishes that artifact via OIDC;
...
```bash
git tag -s mizan-go/v1.2.0 -m "mizan-go v1.2.0"
git push origin mizan-go/v1.2.0
GOPROXY=https://proxy.golang.org go list -m github.com/alshell7/mizan-client-sdks/mizan-go@v1.2.0
```

Do not use a root `v1.2.0` tag for this monorepo layout.
